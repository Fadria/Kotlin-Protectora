...
import com.feadca.protectora.utils.BECOME_VOLUNTEER
...
class MailingViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val volunteerLD: MutableLiveData<String?> = MutableLiveData()
    ...
    fun sendVolunteerMail(email: String) {
        CoroutineScope(Dispatchers.IO).launch {
            ...
            val data = prepareVolunteerParams(email) // Datos a enviar en la petición

            // Convertimos nuestro mapa en un json que enviaremos en la petición
            val jsonObject = JSONObject(data as Map<*, *>?)

            makeVolunteerRequest(url, jsonObject) // Realizamos la petición
        }
    ...
    private fun makeVolunteerRequest(url: String, data: JSONObject) {
        ...
        // Variable que contendrá nuestra petición
        val volunteerRequest: JsonObjectRequest = object : JsonObjectRequest(
            Request.Method.POST,
            url,
            data,
            Response.Listener {
                ...
                if (resultData["status"].toString() == "ok") {
                    // Actualizamos el valor del live data
                    volunteerLD.postValue("Solicitud de voluntariado enviada. Muchas gracias.")
                } else {
                    volunteerLD.postValue("Se ha producido un error, inténtelo de nuevo más tarde")
                }
            },
            Response.ErrorListener { error ->
                Log.i("Se ha producido un error", error.toString())
        ...
        queue.add(volunteerRequest) // Añadimos la petición y la realizamos
    }
}